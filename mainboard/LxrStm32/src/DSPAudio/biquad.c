...
//--------------------------------------------------
void calcBiquadBlock( int16_t* buf, const uint8_t size)
{
	uint8_t i;
	float x;
	biquad * biquadFilter;
	//biquadFilter = BiQuad_new(LPF, dbGain,  freq, srate,  bandwidth);
	biquadFilter = BiQuad_new(LPF, 6.0,  2000.0, 44100.0,  1000.0);
	
	for(i=0;i<size;i++)
	{
			x = buf[i]/32767.f;
			//x = (1+dist->shape)*x/(1+dist->shape*fabsf(x));
			x = BiQuad(x, biquadFilter);
			buf[i] = (x*32767);
	}
}
//--------------------------------------------------
